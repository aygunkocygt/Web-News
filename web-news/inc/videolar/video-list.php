...
<?php
    $VideoList = Sor("SELECT KatID, AltKatID, Title, Url, Description, Img, Tarih FROM videolar_tr ORDER BY ID Desc");
    if(Say($VideoList)>0){
...
<img class="img-responsive " src="images/Video/'.$VideoImg.'" title="'.$VideoListYaz["Img"].'" alt="'.$VideoListYaz["Description"].'" lobgdesc="'.$SiteUrl.'?Gelen=videoDetay&url='.$VideoListYaz["Url"].'" />';
